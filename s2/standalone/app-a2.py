...
def get_song(music_id):
    ...
    if music_id in database:
        ...
        response = {
            "Count": 1,
            "Items": [
                {"Artist": value[0], "SongTitle": value[1], "music_id": music_id}
            ],
        }
    else:
...
def create_song():
    ...
    try:
        ...
        return app.make_response(({"Message": "Error reading arguments"}, 400))
    music_id = str(uuid.uuid4())
    database[music_id] = (Artist, SongTitle)
    response = {"music_id": music_id}
    return response
...
def test():
    ...
    if "1fe6fb91f712c488c2f3f41471236412cc153e099737239114b2ff32bf8e9a53" != ucode:
